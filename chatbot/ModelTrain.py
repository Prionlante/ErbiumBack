...
class ncm():
    ...
    def predict(self, text, num_of_tag):
        p = self.bag_of_words(text, self.words)
        reslist = self.model.predict(np.array([p]))[0]
        tags = []
        f = open("class.txt", "r", encoding="Windows-1251")
        clslist = [line for line in f]
        f.close()
        for i in range(num_of_tag):
            tags.append(clslist[np.argmax(reslist)][:-1])
...
x = ncm()
x.train_model(epoh=50, data_path='ds.xlsx', log=True)
x.save_model('save\\model')
#x.load_model('save\\model')
#print(x.predict("Мы не делаем предварительных прогнозов решения задачи. Для начала необходимо ознакомиться с техническим заданием, после чего мы сформулируем постановку задачи и будем выбирать методы ее решения.", 3))
